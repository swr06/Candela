...
//code by amd and modifications by @goingdigital 
...
vec4 FsrRcasLoadF(sampler2D tex, vec2 p) {
    return texture(tex,p/textureSize(tex,0));
}
...
)
{
    ...
    //    h
    vec2 sp = vec2(ip);
    vec3 b = FsrRcasLoadF(tex,sp + vec2( 0,-1)).rgb;
    vec3 d = FsrRcasLoadF(tex,sp + vec2(-1, 0)).rgb;
    vec3 e = FsrRcasLoadF(tex,sp).rgb;
    vec3 f = FsrRcasLoadF(tex,sp+vec2( 1, 0)).rgb;
    vec3 h = FsrRcasLoadF(tex,sp+vec2( 0, 1)).rgb;
    // Luma times 2.
...
vec3 FSRCAS(sampler2D tex, ivec2 Pixel, float sharpness) {
    float con;
    FsrRcasCon(con,sharpness);
    vec3 col = FsrRcasF(tex, vec2(Pixel), con);
    return col;
}


// EASU 
vec3 FsrEasuCF(sampler2D tex, vec2 p) {
...
)
{
    ...
    vec3 bC = FsrEasuCF(tex, p0 + off.xw); float bL = bC.g + 0.5 *(bC.r + bC.b);
    vec3 cC = FsrEasuCF(tex, p0 + off.yw); float cL = cC.g + 0.5 *(cC.r + cC.b);
    vec3 iC = FsrEasuCF(tex, p1 + off.xw); float iL = iC.g + 0.5 *(iC.r + iC.b);
    vec3 jC = FsrEasuCF(tex, p1 + off.yw); float jL = jC.g + 0.5 *(jC.r + jC.b);
    vec3 fC = FsrEasuCF(tex, p1 + off.yz); float fL = fC.g + 0.5 *(fC.r + fC.b);
    vec3 eC = FsrEasuCF(tex, p1 + off.xz); float eL = eC.g + 0.5 *(eC.r + eC.b);
    vec3 kC = FsrEasuCF(tex, p2 + off.xw); float kL = kC.g + 0.5 *(kC.r + kC.b);
    vec3 lC = FsrEasuCF(tex, p2 + off.yw); float lL = lC.g + 0.5 *(lC.r + lC.b);
    vec3 hC = FsrEasuCF(tex, p2 + off.yz); float hL = hC.g + 0.5 *(hC.r + hC.b);
    vec3 gC = FsrEasuCF(tex, p2 + off.xz); float gL = gC.g + 0.5 *(gC.r + gC.b);
    vec3 oC = FsrEasuCF(tex, p3 + off.yz); float oL = oC.g + 0.5 *(oC.r + oC.b);
    vec3 nC = FsrEasuCF(tex, p3 + off.xz); float nL = nC.g + 0.5 *(nC.r + nC.b);
...
vec3 FSRUpscaleEASU(sampler2D Texture, ivec2 Pixel, vec2 TargetResolution) {
     vec3 c;
     vec4 con0,con1,con2,con3;
    
     vec2 rendersize = textureSize(Texture,0);
     vec2 res = TargetResolution;

     FsrEasuCon(
         con0, con1, con2, con3, rendersize, rendersize, res
     );


     FsrEasuF(Texture, c, Pixel, con0, con1, con2, con3);
     return c;
}